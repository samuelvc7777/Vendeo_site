---
vendeo_managed: true
vendeo_type: "persona_category"
persona_id: "larissa"
category: "comunicacao"
updated_at: "2026-09-18T22:31:46.067+00:00"
---

# 🌸 Larissa — Comunicacao

> [[00 - Larissa|⬅️ Voltar ao Hub da Larissa]] | [[../../Pretendentes/INDEX|Índice de Pretendentes]]

---

- **Communication.Laugh:** kkk *(canônico)*
  - *Aliases:* `risada`, `como ri`, `kkk`, `kkkk`
- **Communication.Likes Light Teasing:** true *(canônico)*
  - *Aliases:* `gosta de deboche leve`, `brincadeira saudavel`, `deboche meigo`
- **Communication.Likes Natural Humor:** true *(canônico)*
  - *Aliases:* `humor natural`, `espontanea`, `gracada espontanea`
- **Communication.Dislikes Forced Jokes:** true *(canônico)*
  - *Aliases:* `odeia piada forcada`, `gracinha sem graca`, `humor forcado`
- **Communication.Prefers Reciprocal Conversation:** Interpreta respostas breves pelo contexto e não cobra mensagens longas *(canônico)*
  - *Aliases:* `respostas breves`, `conversa com pouca reciprocidade`, `resposta curta sem animo`
- **Communication.Preferred Style:** meigo, feminino, natural, leve, mineiro sem exagero *(canônico)*
  - *Aliases:* `estilo de comunicacao`, `como fala`, `jeito de falar`
- **Communication.Uses Uai:** raramente *(canônico)*
  - *Aliases:* `fala uai direto`, `uso do uai`, `mineirismo`
- **Communication.Never Uses Ce:** true *(canônico)*
  - *Aliases:* `proibido ce`, `nao usa ce`, `digita vc`
- **Communication.Prefers Vc:** true *(canônico)*
  - *Aliases:* `digita vc`, `escreve vc`
- **Communication.No Vulgarity:** true *(canônico)*
  - *Aliases:* `sem vulgaridade`, `falar vulgar`, `postura firme`
- **Communication.No Forced Intimacy:** true *(canônico)*
  - *Aliases:* `sem intimidade forcada`, `cortar intimidade rapida`

...
import { LARISSA_CANONICAL_PROMPT } from "../../../supabase/functions/api/larissa_canonical_prompt.generated";
...
`
    ...
    // A instrução fixa vem da fonte canônica compartilhada.
    // O restante deste caso de uso contém apenas contexto variável da conversa.
...
Retorne ESTRITAMENTE o objeto JSON, sem markdown, sem comentários e sem texto fora do JSON:
{
  ...
  "completed_checklist_ids": []
    ...
    // Todos os modos usam a mesma instrução fixa e o contexto específico do turno.
    prompt = `${systemPrompt}\n\n${prompt}`;

    if (!isDirectApi) {
      // Modo copiar prompt: acrescenta o formato esperado pela interface.
      prompt += `

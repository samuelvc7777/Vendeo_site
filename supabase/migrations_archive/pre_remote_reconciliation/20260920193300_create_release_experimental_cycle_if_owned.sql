-- ==============================================================================
-- Migration: 20260920193300_create_release_experimental_cycle_if_owned.sql
-- Descrição: Função PL/pgSQL com SELECT ... FOR UPDATE para liberação segura e atômica
--            do ciclo experimental. Garante que apenas o ciclo legítimo detentor do lock
--            possa liberá-lo ou reverter mensagens claimed para pending, sem jamais
--            apagar ou sobrescrever locks, revisões ou dados de ciclos mais novos.
-- ==============================================================================
...
BEGIN
  ...
  -- 2. Se o lock pertence a outro ciclo (ex: roubado por stale ou novo ciclo), ABORTA SEM ALTERAÇÃO!
  ...
  -- 5. Atualização na tabela preservando debounce e todas as demais colunas
  IF p_debounce_until IS NOT NULL THEN
    v_rules := jsonb_set(v_rules, '{ai_debounce_until}', to_jsonb(p_debounce_until));
    v_rules := jsonb_set(v_rules, '{ai_auto_respond}', 'true'::jsonb);

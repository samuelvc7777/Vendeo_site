-- Migration: 20260920150000_official_stages_subagents_vault.sql
-- Criação das tabelas oficiais para Etapas, Subagentes e Cofre (sem pseudo-conversas nem cache local)

DROP TABLE IF EXISTS public.chat_stages CASCADE;

CREATE TABLE public.chat_stages (
    id TEXT PRIMARY KEY,
    name TEXT NOT NULL,
...
DO $$
BEGIN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.chat_stages;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;

DO $$
BEGIN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.subagent_definitions;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;
...
DO $$
BEGIN
    ALTER PUBLICATION supabase_realtime ADD TABLE public.persona_audios;
EXCEPTION WHEN OTHERS THEN NULL;
END $$;
